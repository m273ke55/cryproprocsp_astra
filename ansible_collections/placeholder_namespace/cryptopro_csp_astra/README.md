...
## License и smoke-tests

- Для применения лицензии задайте:
  - `cryptopro_csp_license_apply: true`
  - `cryptopro_csp_license_key: "<ключ>"` (через Vault).
- Лицензия проверяется командой `cpconfig -license -view`; если уже активна, повторный прогон не меняет систему.
- `cryptopro_csp` smoke-test проверяет наличие `cpconfig`, `certmgr`, `csptest` и выполняет read-only команду `cpconfig -license -view`.
- `cryptopro_csp_token` устанавливает пакеты `cryptopro_csp_token_packages`, запускает/включает `pcscd` и проверяет, что сервис активен.
